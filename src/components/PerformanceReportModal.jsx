...
export default function PerformanceReportModal({ user, state, onClose }) {
  const xp = state.xp || 0
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
          <div style={{ background: 'rgba(245, 158, 11, 0.12)', border: '1px solid #f59e0b', padding: 12, borderRadius: 14, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 800 }}>TOTAL XP</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#fbbf24' }}>{xp}</div>
          </div>
          <div style={{ background: 'rgba(16, 185, 129, 0.12)', border: '1px solid #10b981', padding: 12, borderRadius: 14, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 800 }}>QUIZ SCORE</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#6ee7b7' }}>{quizScore}%</div>
          </div>
          <div style={{ background: 'rgba(56, 189, 248, 0.12)', border: '1px solid #38bdf8', padding: 12, borderRadius: 14, textAlign: 'center' }}>
            <div style={{ fontSize: 10, color: '#9ca3af', fontWeight: 800 }}>SIMS CLEARED</div>
            <div style={{ fontSize: 22, fontWeight: 900, color: '#a5f3fc' }}>{completedModules}/8</div>
          </div>
        </div>

        {/* FR19: Progress Trends */}
        <div style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(255,255,255,0.1)', padding: 16, borderRadius: 16, marginBottom: 20 }}>
          <h4 style={{ fontSize: 12, fontWeight: 900, color: '#fbbf24', marginBottom: 10, textTransform: 'uppercase' }}>
            📈 Learning Progress Breakdown
          </h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 12 }}>
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span>Video Lessons (Level 1)</span>
              ...
              <div style={{ width: '100%', height: 6, background: 'rgba(255,255,255,0.1)', borderRadius: 999 }}>
                <div style={{ width: `${watchedCount * 25}%`, height: '100%', background: '#f59e0b', borderRadius: 999 }} />
              </div>
            </div>

            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span>Investment Lab Simulators (Level 2)</span>
                <strong>{completedModules}/8 ({Math.round((completedModules/8)*100)}%)</strong>
              </div>
              <div style={{ width: '100%', height: 6, background: 'rgba(255,255,255,0.1)', borderRadius: 999 }}>
                <div style={{ width: `${Math.round((completedModules/8)*100)}%`, height: '100%', background: '#38bdf8', borderRadius: 999 }} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ background: 'rgba(225, 29, 72, 0.12)', border: '1px solid rgba(225, 29, 72, 0.4)', padding: 16, borderRadius: 16, marginBottom: 20 }}>
          <h4 style={{ fontSize: 12, fontWeight: 900, color: '#fda4af', marginBottom: 8, textTransform: 'uppercase' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={handlePrintReport} className="btn-outline" style={{ flex: 1, padding: 12, fontSize: 12 }}>
